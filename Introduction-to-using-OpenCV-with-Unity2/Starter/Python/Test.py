...
while capture.isOpened():
    ...
    ret,thresh = cv2.threshold(filtered, 127, 255, 1)
    
    # Show threshold image
    cv2.imshow("Thresholded2", thresh)

    ret,thresh = cv2.threshold(filtered, 127, 255, 0)
    
    # Show threshold image
    cv2.imshow("Thresholded3", thresh)

    ret,thresh = cv2.threshold(filtered, 50, 127, 1)
    
    # Show threshold image
    cv2.imshow("Thresholded4", thresh)

    #print("size",np.size(filtered))

    ret,thresh = cv2.threshold(filtered, 127, 200, 0)
