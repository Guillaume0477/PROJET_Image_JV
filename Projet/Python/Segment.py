...
def getHSVColorSeg(im, bounds, refColor, toleranceH = 20):
    # toleranceH = 10
    toleranceS = 100
    toleranceV = 60
...
def getHSVBGRYUVColorSeg(im, bounds, refColorBGR, refColorHSV, YUV_Value):
    ...
    w=np.array([1,1,1,1,1,1,1,1,1])#BGRHSVYUV
    ...
    print("taille current ",np.shape(current))
    print("taille refcolor ",np.shape(refcolor))
    print("w ",np.shape(w))

    frame_norm = getnorm_ND(refcolor,current,w,9)
    print("taille norm",np.shape(frame_norm))
